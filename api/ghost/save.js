...
export default async function handler(req, res) {
    try {
        ...
        if (!authHeader || authHeader !== `Bearer ${SECRET_KEY}`) {
            ...
            return res.status(401).json({ message: 'Unauthorized: Ghost Key mismatch.' });
        }
        ...
        console.error('[Ghost Server CRASH]', criticalError);
        return res.status(500).json({
            message: `Server Crash: ${criticalError.message}`,
